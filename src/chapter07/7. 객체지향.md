...
- 기본생성자 생략 시 JVM이 자동으로 호출하여 실행됨(단, 정의된 생성자가 존재하지 않는 경우)
- 호출위치는 new 생성자(); 
- 생성자 이름은 클래스명과 동일
...
- static : 클래스의 필드, 메소드에 정의되며 객체를 생성하지 않고 직접 클래스명을 통해 접근이 가능함
  ...
  호출 방법 :	(1) 클래스명.필드명, 클래스명.메소드명() => 클래스 변수, 클래스 메소드
  		   	(2) 객체명.필드명, 객체명.메소드명()
...
#### 8-4) 싱글톤(Singleton)
- 디자인 패턴(Design Pattern) 중에 하나로 객체를 하나만 생성해놓고, 메소드를 통해 클래스타입으로 리턴하는 형식의 디자인을 말함
