...
public class OrderSystem {
	...
	public void processPayment() {
		...
		if(scan.hasNextInt()) {
			...
			System.out.println("=> 총 입금 금액 : "+String.format("%,d", payment.getAmount())+"원");
			if(payment.getPayment(order.orderMenu.getPrice())) {
				// 결제 완료
				System.out.println("=> 결제 완료!! 잔돈 : "+String.format("%,d", payment.getChange())+"원");
				showMenu();
				selectMenu();
			} else {
				// 결제 금액 부족
	...
	public Menu searchMenu(int menuNo) {
		...
		for(Menu m : menuList) {	// menuList[0] --> m(주소값) --> Menu(아메리카노);
			if(m.getNo() == menuNo) {
				menu = m;
				break;
			}
